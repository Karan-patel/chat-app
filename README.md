...
- **Slim Framework**: A micro-framework (v4.x) for routing and middleware, chosen for its lightweight nature and flexibility.
...
- **SOLID Principles**:
    - *Single Responsibility Principle (SRP)*: Each class has a single purpose (e.g., `GroupController` for group operations).
...
The database schema is defined in `schema.sql`, read during initialization, ensuring maintainability. The schema includes:
...
- `messages`: Stores messages with timestamps, using SQLite’s `CURRENT_TIMESTAMP` for automatic setting, and foreign keys to `groups` and `users`.

The `Database` class uses PDO with prepared statements to prevent SQL injection, ensuring security. Queries are optimized for efficiency, such as joining tables in `getMessagesByGroup` to fetch sender usernames in one call, avoiding N+1 query problems.
...
The `UserMiddleWare` extracts the `X-Username` header, creates the user if not exists, and attaches the user ID to the request, following the Chain of Responsibility pattern for middleware processing.

## Deployment

#### Using PHAR File
The PHP equivalent of a JAR file is a PHAR file, created using PHP’s `Phar` class. The `create-phar.php` script builds `chat-app.phar`, including:
- `app.php`, `src/`, `vendor/`, and `schema.sql`.
- Excludes `tests/` and `chat.db`, as the latter must be separate for write operations.
